...
insert into nascidos select name,fk_snapshot_git_date from snapshot_atual where name NOT IN (select name from snapshot_anterior);
insert into falecidos select name,fk_snapshot_git_date from snapshot_anterior where name NOT IN (select name from snapshot_atual); 
insert into top_terms select * from terms where fk_snapshot_git_date=data_atual ORDER BY quantity DESC LIMIT 10;
END $$ 
DELIMITER ;
